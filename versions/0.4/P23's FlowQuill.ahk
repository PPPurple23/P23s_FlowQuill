...
Loop, %0%  ; 드롭된 파일의 개수만큼 루프
{
    filedropped .= %A_Index% " "  ; 각각의 파일 경로 부분을 추가
	break
}
...
appversion = 0.4
appbuilddate = 2025년 1월 1일
appupdatedetails = Run 탭 추가

listvar := {}

/*
what := listvar.MaxIndex()
yes := listvar.1
MsgBox, yes: %yes%`nwhat: %what%
...
Menu, InfoMenu, Add, &Mouse 문법 정리본	Ctrl+F1, 			OpenMouseDocs
Menu, InfoMenu, Add, &Wikipedia의 베지에 곡선	Ctrl+F2, 	OpenBezierDocs
Menu, InfoMenu, Add, &Key Send 문법 정리본	Ctrl+F3, 		OpenSendDocs
Menu, InfoMenu, Add, &SoundPlay 문법 정리본	Ctrl+F4, 		OpenSoundPlayDocs
Menu, InfoMenu, Add, &Message Box 문법 정리본	Ctrl+F5, 	OpenMsgBoxDocs
Menu, InfoMenu, Add
Menu, InfoMenu, Add, &Loop 문법 정리본	Ctrl+F6, 			OpenLoopDocs
Menu, InfoMenu, Add, &Delay 문법 정리본	Ctrl+F7, 			OpenDelayDocs
Menu, InfoMenu, Add, &Window 관련 문법 정리본	Ctrl+F8, 	OpenWindowDocs
Menu, InfoMenu, Add
...
Menu, EditMenu, Add
Menu, EditMenu, Add, &라인 삭제	Delete,			BTN_Delete
...
Gui, Tab, Delay
Gui, Add, Text, x632 y59 w470 h60, Delay 도움말:`n입력한 대기 시간 동안 대기합니다.`n`n단위는 ms(mili second)로`, 1초가 1000ms(밀리초)입니다.
Gui, Add, GroupBox, x632 y129 w170 h50, 대기 시간
Gui, Add, Edit, x642 y149 w122 h20 +Center -VScroll vED_DLY gSafety, 250
Gui, Add, Text, x766 y149 w35 h20 0x200, ms
Gui, Add, Button, x812 y129 w290 h50 vBTN_DLY gBTN_DLY, 250ms 대기 추가
...
else
{
	...
	{
		GuiControl, , BTN_DLY, %ED_DLY%ms 대기 추가
	}
	if (A_GuiControl = "ED_LCT" && A_GuiContent > 999)
	{
...
if (script_working = 1)
	return
ListBox_Batch_Process("[DELAY]  " . ED_DLY, "LineList", ED_DLY . "ms 딜레이 추가")
return
...
/*
	...
	ED_WNM ; 창 이름을 담을 edit
	BTN_GWN ; 창 이름 가져오기 버튼
	ED_WWS ; WaitSeconds
	
	BTN_W? ; 순서대로 activate, close, kill
	wait, waitactivate, waitnotactivate
...
AppInfo:
MsgBox, 4160, %appname% [v%appversion%] - 정보, 제작: P23 (PPPurple23)`n`n문의 연락처: pppurple23@proton.me`n`n빌드된 날짜: %appbuilddate%`n버전: %appversion%`n`n해당 버전의 개선점: %appupdatedetails%
return
...
Loop
{
	...
	; 명령어 / 파라미터 토큰화 디버깅용
		...
		Loop
		{
			temp_debug := param%A_Index%
			if (temp_debug = "")
			{
				break
			}
			WhatToShow = %WhatToShow%`nParam#%A_Index%:		%temp_debug%
		}
		MsgBox, %WhatToShow%
		*/
	; 디버깅용 끝
