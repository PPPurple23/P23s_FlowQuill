...
appversion = 0.41.1
appbuilddate = 2025년 1월 5일
appupdatedetails = `n*Delay 탭에 무작위 딜레이 추가.`n*Delay 탭에 밀리초/초/분 단위 변환기 추가.`n`n*연결 파일에 공백이 포함되면`, 인식하지 못하던 문제 수정.`n*기본 메뉴바/Info에 Github 방문 메뉴 추가.`n*프로그램의 단일 실행 제한 해제.`n`n*라인 삭제 단축키 변경: Delete -> Ctrl+Delete`n`n`n핫픽스 (1):`nMsgBox로 줄바꿈이 안 되던 문제 수정.
